...
results_file = 'outdir_symmetry_energy_100likes_sigma4.0/dynesty_result.json'
...
def neutron_fraction_(xarr, E_0, K_0, J_0, S_0, L_0, K_sym, J_sym):
    return neutron_fraction(xarr, S_0, L_0, K_sym, J_sym)


ci, ys = calc_confidence_interval(results_file, x, neutron_fraction_,
                                  ndraws=100)
fig2 = plt.subplots(figsize=(6, 5))
for y in ys[0]:
    fig2[1].plot(rhox, 1-y, color='grey', alpha=0.3, linewidth=1, zorder=0)
# fig2[1].fill_between(rhox, 1-ci[:, 0], 1-ci[:, 1], color='grey',
#                     alpha=0.5, label=r'$E_{\rm sym}$ ${\rm inference}$')
fig2[1].set_xlim(min(rhox), max(rhox))
# fig2[1].set_yscale('log')
fig2[1].set_xlabel(r'$\rho$ (${\rm kg}$ ${\rm m}^{-3}$)')
fig2[1].set_ylabel(r'$f_p$')
fig2[0].tight_layout()
...
#     priors["E_0"] = np.random.uniform(-25, 0)
#     priors["K_0"] = np.random.uniform(150, 300)
#     priors["J_0"] = np.random.uniform(-200, 200)
#     priors["S_0"] = np.random.uniform(20, 40)
#     priors["L_0"] = np.random.uniform(30, 80)
#     priors["K_sym"] = np.random.uniform(-200, 0)
#     priors["J_sym"] = np.random.uniform(-200, 200)
#     m = dimensionless_energy_per_baryon_model(x, **priors)
#     if np.all((m[2:] > ci[2:, 0]) & (m[2:] < ci[2:, 1])):
#         for key in priors.keys():
#             priors_inside[key].append(priors[key])
#         fig[1].plot(rhox, m, color='grey',
#                     alpha=0.3, linewidth=1, zorder=0)
# for key in priors_inside.keys():
#     priors_inside[key] = np.mean(priors_inside[key])
# print(priors_inside)
...
fig[0].tight_layout()
# fig[0].savefig('symmetry_energy_confidence_interval.pdf')
